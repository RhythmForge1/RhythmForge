...
//   }, []);
   ...
   useEffect(() => {
    if (projectCode) {
      axios
        .get(`http://localhost:8080/api/deliverables/${projectCode}`)
        .then((response) => {
...
const token = userDetails?.jwtToken; // Extract JWT token
    axios
    .patch(
      `http://localhost:8080/api/projects/${projectCode}/stages/${newStage}`,
      { status: "In Progress" },
      {
        method: "PATCH",
        headers: {
...
const updateStatus = async (newStatus) => {
 ...
 if (milestoneId) {
  const response = await axios.patch(
    `http://localhost:8080/api/milestones/${milestoneId}/complete`,
  ...
  setCurrentCycleVoucher(updatedUser.currentCycleVoucher);
    ...
    await axios.patch(
      `http://localhost:8080/api/projects/${projectCode}/stages/${currentStage}`,
      { status: newStatus },
      { headers: { Authorization: `${token}` } }
    );
...
const fetchLatestProjectData = async () => {
  try {
    ...
    const response = await axios.get(`http://localhost:8080/api/projects/${projectCode}`, {
      headers: { Authorization: `${token}` }
    });
...
const updateStageDates = () => {
  ...
  axios
    .patch(
      `http://localhost:8080/api/projects/${projectCode}/stages/${currentStage}/dates`,
      { startDate, endDate },
      {
        headers: {
          Authorization: `${token}`,
...
useEffect(() => {
    if (projectCode) {
      ...
      const fetchProjectDetails = async () => {
        try {
          ...
          const response = await fetch(`http://localhost:8080/api/projects/${projectCode}`, {
            method: "GET",
            headers: {
  ...
  useEffect(() => {
    fetch("http://localhost:8080/api/deliverables")
      .then((res) => res.json())
      .then((data) => {
